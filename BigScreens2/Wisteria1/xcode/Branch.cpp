...
: mOrigin( aOrigin )
{
    mIsRoot = true;
    float baseHue = 1.0f;
    float sat = 0.0 + Rand::randFloat( 0.0f );
    float brightness = 0.5 + Rand::randFloat( -0.3, 0.5f );
    ...
    if( ( mLifespan > 0 ) && ( ! mIsRoot ) ) {
        ...
        if( Rand::randFloat() < mFlowerProb * ( 1.0 - ageLerp ) ) {
            if( mBranches.size() < MAX_BRANCHES ) {
                ...
                newBranch.mLifespan = (int)(30000 * mScale);
                newBranch.mTotalLifespan = newBranch.mLifespan;
                newBranch.mLaunchDelay = Rand::randInt( 40, 500 );
...
void Branch::createBranch( const vec2 &origin, float baseHue, ColorA flowerColor, int launchDelay )
{
    ...
    int lifespan = 1000 + Rand::randInt( 100 );
    float speed = Rand::randFloat( 0.1f, 0.3f );
...
float Branch::randomHue()
{
    if( Rand::randFloat( 0.6666666666f ) > 0.5f ) // in [ 0, 60 ]
        return Rand::randFloat( 360.0f / 360.0f, 360.0f / 360.0f );
    else
        return Rand::randFloat( 360.0f / 360.0f, 360.0f / 360.0f );
}